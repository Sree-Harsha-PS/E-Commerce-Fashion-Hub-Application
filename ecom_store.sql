...
ALTER TABLE `customer_orders`
ADD FOREIGN KEY (EnquiryTypeID) REFERENCES EnquiryType(EnquiryTypeID);
...
-- Queries using Joins, Set Operations, and Aggregate Functions
-- Queries using Joins
...
UNION
SELECT ProductName FROM ArchivedProduct;
...
FOR EACH ROW
BEGIN
   -- For example, you can update the total amount in the associated order
   UPDATE `Order`
...
BEGIN
   -- For example, you can calculate the total amount for a specific order
...
BEGIN
   RETURN Price * 0.10; -- Assuming a 10% tax rate
END;
//
DELIMITER ;
